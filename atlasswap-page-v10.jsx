...
async function fetchBestRate(from, to, amount) {
  try {
    ...
    const data = await res.json();
    if (!res.ok || !data?.ok) throw new Error(data?.error || "Rate API failed");
    return {
      best: data.best,
    ...
    return {
      best: { provider: "ChangeNOW", rate: fallback, available: true, simulated: true },
      comparison: [{ provider: "ChangeNOW", rate: fallback, rawRate: fallback / amount, available: true, simulated: true }],
      minAmount: parseFloat((2 / (BASE_RATES[from] || 1)).toPrecision(4)),
    };
  }
}
...
async function createExchange(provider, from, to, amount, destAddress, extraData = {}) {
  ...
  const data = await res.json();
  if (!res.ok || !data?.ok) throw new Error(data?.error || "Create API failed");
  return data.result;
}
