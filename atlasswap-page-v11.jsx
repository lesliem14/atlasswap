...
const CN_KEY  =
  ...
  "";
const SS_KEY  =
  process.env.NEXT_PUBLIC_EXOLIX_API_KEY ||
  process.env.NEXT_PUBLIC_EXOLIX_KEY ||
...
const SS_BASE = "https://exolix.com/api/v2";
const SZ_BASE = "https://api.swapzone.io/v1";

// ── Exolix network mapping ──
...
const SS_NETWORKS = {
  ...
  // DeFi tokens (all ERC-20 unless noted)
  ...
  KSM:"ksm",    ZEN:"zen",      DCR:"dcr",     RVN:"rvn",     SC:"sc",
  DGB:"dgb",
};
...
//    Rate:   GET  V2 /exchange/estimated-amount  (header auth)
//    Min:    GET  V1 /min-amount/{from}_{to}     (real minimum per pair)
//    Create: POST V1 /transactions/{key}         (key in URL — proven partner endpoint)
// ─────────────────────────────────────────────────────────────
async function fetchChangeNowRate(from, to, amount) {
  try {
    if (!CN_KEY) throw new Error("No CN key");

    // Fetch rate and real minimum simultaneously
    const [rateRes, minRes] = await Promise.all([
      fetch(`${CN_V2}/exchange/estimated-amount?fromCurrency=${from.toLowerCase()}&toCurrency=${to.toLowerCase()}&fromAmount=${amount}&flow=standard&type=direct`,
        { headers: { "x-changenow-api-key": CN_KEY } }),
      fetch(`${CN_V1}/min-amount/${from.toLowerCase()}_${to.toLowerCase()}?api_key=${CN_KEY}`),
    ]);

    if (!rateRes.ok) throw new Error(`CN rate ${rateRes.status}`);
    const rateData = await rateRes.json();
    const estimated = parseFloat(rateData?.toAmount || rateData?.estimatedAmount || 0);
    if (!estimated || isNaN(estimated)) throw new Error("No CN amount");

    // Real minimum — e.g. { minAmount: 0.0137501 }
    let minAmount = 0;
    if (minRes.ok) {
    ...
    return {
      provider: "ChangeNOW", rate: estimated, rawRate: estimated / amount,
      minAmount, available: true, simulated: false,
    };
  } catch {
...
async function createExchangeChangeNow(from, to, amount, destAddress) {
  ...
  if (!destAddress || !destAddress.trim()) throw new Error("No destination address provided");
  if (!from || !to || !amount || amount <= 0) throw new Error("Missing required fields: from, to, amount");
  ...
  if (!res.ok) {
    // Extract real minimum from ChangeNOW error message if present
    // e.g. "Amount is less than minimal: 0.0137501 SOL"
  ...
  const depositAddress = data?.payinAddress || data?.payin?.address || data?.deposit_address || "";
  const exchangeId     = data?.id || data?.requestId || "";
  const payinExtraId   = data?.payinExtraId || data?.payin?.extraId || "";

  if (!depositAddress) {
    console.error("ChangeNOW response (no address):", JSON.stringify(data));
    throw new Error(`ChangeNOW no deposit address: ${JSON.stringify(data)}`);
  }
...
async function createExchangeExolix(from, to, amount, destAddress) {
  if (!SS_KEY) throw new Error("No Exolix key");
  const netFrom = SS_NETWORKS[from] || from.toLowerCase();
  const netTo   = SS_NETWORKS[to]   || to.toLowerCase();
  ...
  if (!depositAddress) {
    console.error("Exolix response:", JSON.stringify(data));
    throw new Error(`Exolix no deposit address: ${JSON.stringify(data)}`);
  }
...
//    Rate: GET /v1/exchange/get-rate → returns quotaId needed for create
//    Create: POST /v1/exchange/create (must include quotaId)
//    Auth: header x-api-key + body apikey
// ─────────────────────────────────────────────────────────────
async function fetchSwapzoneRate(from, to, amount) {
  try {
    ...
    const res = await fetch(url, { headers: { "x-api-key": SZ_KEY } });
    if (!res.ok) throw new Error(`SZ rate ${res.status}`);
    const data = await res.json();
    const amountTo = parseFloat(data?.amountTo ?? 0);
    if (!amountTo || isNaN(amountTo)) throw new Error("No SZ amount");
    return {
      provider: "Swapzone", rate: amountTo, rawRate: amountTo / amount,
      quotaId: data?.quotaId || "", adapter: data?.adapter || "",
...
async function createExchangeSwapzone(from, to, amount, destAddress, quotaId) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(`Swapzone ${res.status}: ${JSON.stringify(data)}`);
  ...
  if (!depositAddress) {
    console.error("Swapzone response:", JSON.stringify(data));
    throw new Error(`Swapzone no deposit address: ${JSON.stringify(data)}`);
  }
  return { depositAddress, exchangeId, payinExtraId, provider: "Swapzone" };
}

// ─────────────────────────────────────────────────────────────
// AGGREGATOR — all 3 simultaneously, returns best + real minAmount
// ─────────────────────────────────────────────────────────────
async function fetchBestRate(from, to, amount) {
  const [cn, ss, sz] = await Promise.allSettled([
    fetchChangeNowRate(from, to, amount),
    fetchExolixRate(from, to, amount),
  ...
  // Use real minimum from ChangeNOW (most reliable source), fallback to $2 USD equivalent
  const cnResult  = cn.status === "fulfilled" ? cn.value : null;
  const realMin   = cnResult?.minAmount || 0;
  // Add 10% buffer so users don't hit edge cases right at the minimum
  ...
  return { best, comparison: results, minAmount };
}
...
async function createExchange(provider, from, to, amount, destAddress, extraData = {}) {
  const order = [provider, ...["ChangeNOW", "Exolix", "Swapzone"].filter(p => p !== provider)];
  let lastErr = null;
  for (const p of order) {
    try {
      if (p === "ChangeNOW")  return await createExchangeChangeNow(from, to, amount, destAddress);
      if (p === "Exolix") return await createExchangeExolix(from, to, amount, destAddress);
...
async function getTransactionStatus(provider, exchangeId) {
  try {
    if (provider === "ChangeNOW") {
      const res = await fetch(`${CN_V1}/transactions/${exchangeId}/${CN_KEY}`);
      const data = await res.json();
      return data?.status || "unknown";
    }
    if (provider === "Swapzone") {
      const res = await fetch(`${SZ_BASE}/exchange/tx?id=${exchangeId}`, { headers: { "x-api-key": SZ_KEY } });
      const data = await res.json();
...
export default function AtlasSwapApp() {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  const refreshEvery = 15000; // ms — change this to adjust refresh interval
  ...
  // Fetch USD price for a single coin via ChangeNOW:
  // GET /v1/exchange-amount/1/{symbol}_usdt?api_key=KEY
  // Returns { estimatedAmount: X } where X is the USDT value of 1 coin
  const fetchCNPrice = useCallback(async (symbol) => {
    if (!CN_KEY) return null;
    try {
      const res = await fetch(
        `${CN_V1}/exchange-amount/1/${symbol.toLowerCase()}_usdt?api_key=${CN_KEY}`,
        { signal: AbortSignal.timeout(5000) }
      );
      if (!res.ok) return null;
      const data = await res.json();
      const price = parseFloat(data?.estimatedAmount || 0);
      return price > 0 ? price : null;
    } catch { return null; }
  }, []);

  // Fetch USD price via Exolix as fallback:
  // GET /v3/estimates?tickerFrom={sym}&networkFrom={net}&tickerTo=usdt&networkTo=eth&amount=1
  const fetchSSPrice = useCallback(async (symbol) => {
    if (!SS_KEY) return null;
    try {
      const netFrom = SS_NETWORKS[symbol] || symbol.toLowerCase();
      const res = await fetch(
        `${SS_BASE}/estimates?tickerFrom=${symbol.toLowerCase()}&networkFrom=${netFrom}&tickerTo=usdt&networkTo=eth&amount=1&fixed=false`,
        { headers: { "x-api-key": SS_KEY, "Accept": "application/json" },
          signal: AbortSignal.timeout(5000) }
      );
      if (!res.ok) return null;
      const data = await res.json();
      const price = parseFloat(data?.result?.amountTo ?? data?.result ?? 0);
      return price > 0 ? price : null;
    } catch { return null; }
  }, []);

  // Fetch all ticker prices — CN first, SS as fallback, BASE_RATES as final fallback
  const fetchLivePrices = useCallback(async () => {
    ...
    // Fetch remaining ticker coins, batched with small delays to avoid hammering
    const toFetch = TICKER_SYMBOLS.filter(s => !["USDT","USDC","DAI","BUSD","TUSD"].includes(s));

    // Run in parallel with Promise.allSettled so one failure doesn't block others
    const results = await Promise.allSettled(
      toFetch.map(async (symbol) => {
        // Try ChangeNOW first
        let price = await fetchCNPrice(symbol);
        // If CN fails, try Exolix
        if (!price) price = await fetchSSPrice(symbol);
        // Final fallback: BASE_RATES
        if (!price) price = BASE_RATES[symbol] || 0;
        return { symbol, price };
      })
    );
    ...
    // Compute 24h-style change: compare new price vs previous cycle price
    const newChanges = {};
    ...
    // Update previous prices reference for next cycle's change calculation
    prevPrices.current = { ...prevPrices.current, ...newPrices };

    // Merge with existing prices so coins not in ticker retain their BASE_RATES value
    setTickerPrices(prev => ({ ...prev, ...newPrices }));
    setTickerChanges(prev => ({ ...prev, ...newChanges }));
  }, [fetchCNPrice, fetchSSPrice]);

  // ── Auto-refresh on mount + every refreshEvery ms ──
  useEffect(() => {
    fetchLivePrices(); // immediate on mount
    tickerTimer.current = setInterval(fetchLivePrices, refreshEvery);
    return () => clearInterval(tickerTimer.current);
  }, [fetchLivePrices, refreshEvery]);

  // ── Re-fetch prices instantly when coin or deposit amount changes ──
  // Keeps the USD estimation responsive to pair/amount edits
  useEffect(() => {
    fetchLivePrices();
  }, [fromCoin, toCoin, sendAmt, fetchLivePrices]);
  ...
  const [lastRefreshed, setLastRefreshed] = useState(null); // timestamp of last live rate fetch

  // ── Background rate fetch — all 3 APIs silently ──
  // Also extracts implied USD prices from swap rates and updates ticker
  const fetchRates = useCallback(async (from, to, amount) => {
    ...
    try {
      ...
      // ── Derive live USD prices from swap rate responses ──────────────
      // If we swapped X FROM → Y TO, and the rate is R (amount of TO received per FROM),
      // and TO is USDT, then USD price of FROM = R.
      // More generally: if we know the USD price of TO (from tickerPrices),
      // then USD price of FROM ≈ (R * USD_price_TO) / amount_FROM
      // We use ChangeNOW rate as the most reliable price source.
      const cnResult = comp.find(r => r.provider === "ChangeNOW");
      if (cnResult && cnResult.rawRate > 0) {
        setTickerPrices(prev => {
          const toUSD  = prev[to]   || BASE_RATES[to]   || 1;
          const fromUSD = prev[from] || BASE_RATES[from] || 1;
          // Implied price of FROM in USD: rate_in_TO × USD_per_TO / 1_unit_from
          const impliedFromUSD = cnResult.rawRate * toUSD;
          // Only update if the implied price is within 20% of previous value (sanity check)
          const sanityOk = Math.abs(impliedFromUSD - fromUSD) / fromUSD < 0.20;
          if (impliedFromUSD > 0 && sanityOk) {
            return { ...prev, [from]: impliedFromUSD };
          }
          return prev;
        });
      }
  ...
  // ── Debounced rate fetch on input change (instant re-fetch when coin or amount changes) ──
  useEffect(() => {
  ...
  // ── Auto-refresh swap rate every 15 seconds when on form step ──
  // Keeps the displayed rate and provider fresh without user interaction
  const autoRefreshTimer = useRef(null);
  useEffect(() => {
    if (step !== "form") return; // only refresh on the form step
                ...
                <div style={{ marginBottom: "16px" }}>
                  ...
                  <div style={{
                    ...
                    <span style={{ color: "rgba(0,229,160,0.5)", display: "flex", alignItems: "center", gap: "6px" }}>
                      {/* Live pulse dot — animates when rateLoading */}
                      <span style={{
                        width: 6, height: 6, borderRadius: "50%",
                        background: rateLoading ? "#F7931A" : "#00E5A0",
                        display: "inline-block",
                        animation: rateLoading ? "spin 0.8s linear infinite" : "none",
                        transition: "background 0.3s",
                      }}/>
                      <span>
                        {rateLoading ? "Refreshing…" : `Best rate via ${bestProvider}`}
                      </span>
                      {lastRefreshed && !rateLoading && (
                        <span style={{ fontSize: "10px", color: "rgba(240,244,255,0.25)", marginLeft: "2px" }}>
