...
// Cache key: "FROM_TO_AMOUNT"
// Entries expire after CACHE_TTL ms — prevents duplicate API
// calls within the same refresh window.
const QUOTE_CACHE     = new Map();   // key → { data, expiresAt }
const CACHE_TTL       = 8000;        // 8 s — expire before next 15s refresh
const REFRESH_INTERVAL = 15000;      // 15 s auto-refresh
const DEBOUNCE_MS     = 400;         // debounce user input before fetching
const MAX_RETRIES     = 1;           // one retry per provider per cycle
...
function setCache(from, to, amount, data) {
  ...
  // Prevent unbounded memory growth — prune entries older than 60 s
...
//    Min:    GET V1 /min-amount/{from}_{to}     (real pair minimum)
//    Create: POST V1 /transactions/{key}        (key in URL — partner endpoint)
// ─────────────────────────────────────────────────────────────
async function fetchChangeNowRate(from, to, amount) {
  const fetchFn = async () => {
    ...
    const [rateRes, minRes] = await Promise.all([
      fetch(
        ...
        { headers: { "x-changenow-api-key": CN_KEY }, signal: AbortSignal.timeout(7000) }
      ),
      fetch(
    ...
    if (!rateRes.ok) throw new Error(`CN rate HTTP ${rateRes.status}`);
    const rateData = await rateRes.json();
    const amountOut = parseFloat(rateData?.toAmount || rateData?.estimatedAmount || 0);
    if (!amountOut || isNaN(amountOut)) throw new Error("CN returned no amount");
    ...
    return {
      ...
      estimatedTime: "2–5 min", fees: 0.4,
      minAmount, quotaId: "", simulated: false, available: true, error: null,
    };
  };
  ...
  try {
    ...
    const fallback = ((amount * (BASE_RATES[from] || 1)) / (BASE_RATES[to] || 1)) * 0.996;
    return {
      provider: "ChangeNOW", fromToken: from, toToken: to,
      amountIn: amount, amountOut: fallback, rate: fallback / amount,
      estimatedTime: "2–5 min", fees: 0.4,
      minAmount: 0, quotaId: "", simulated: true, available: true,
      error: err.message,
    };
  }
}
...
async function fetchExolixRate(from, to, amount) {
  const fetchFn = async () => {
    ...
    const res = await fetch(
      `${SS_BASE}/estimates?tickerFrom=${from.toLowerCase()}&networkFrom=${netFrom}&tickerTo=${to.toLowerCase()}&networkTo=${netTo}&amount=${amount}&fixed=false`,
      { headers: { "x-api-key": SS_KEY, "Accept": "application/json" }, signal: AbortSignal.timeout(7000) }
    );
    if (!res.ok) throw new Error(`SS rate HTTP ${res.status}`);
    const data = await res.json();
    const amountOut = parseFloat(data?.result?.amountTo ?? data?.result ?? 0);
    if (!amountOut || isNaN(amountOut)) throw new Error("SS returned no amount");
    return {
      provider: "Exolix", fromToken: from, toToken: to,
      amountIn: amount, amountOut, rate: amountOut / amount,
      estimatedTime: "5–20 min", fees: 0.4,
      minAmount: 0, quotaId: "", simulated: false, available: true, error: null,
    };
  };
  ...
  try {
    ...
    const fallback = ((amount * (BASE_RATES[from] || 1)) / (BASE_RATES[to] || 1)) * 0.992;
    return {
      provider: "Exolix", fromToken: from, toToken: to,
      amountIn: amount, amountOut: fallback, rate: fallback / amount,
      estimatedTime: "5–20 min", fees: 0.4,
...
//    Rate: GET /v1/exchange/get-rate → returns quotaId
//    Create: POST /v1/exchange/create (quotaId required)
...
async function fetchSwapzoneRate(from, to, amount) {
  const fetchFn = async () => {
    ...
    const res = await fetch(
      ...
      { headers: { "x-api-key": SZ_KEY }, signal: AbortSignal.timeout(7000) }
    );
    if (!res.ok) throw new Error(`SZ rate HTTP ${res.status}`);
    const data = await res.json();
    const amountOut = parseFloat(data?.amountTo ?? 0);
    if (!amountOut || isNaN(amountOut)) throw new Error("SZ returned no amount");
    return {
      provider: "Swapzone", fromToken: from, toToken: to,
      amountIn: amount, amountOut, rate: amountOut / amount,
      estimatedTime: "5–30 min", fees: 0.4,
      minAmount: 0, quotaId: data?.quotaId || "", simulated: false, available: true, error: null,
    };
  };
  ...
  try {
    ...
    const fallback = ((amount * (BASE_RATES[from] || 1)) / (BASE_RATES[to] || 1)) * 0.989;
    return {
      provider: "Swapzone", fromToken: from, toToken: to,
      amountIn: amount, amountOut: fallback, rate: fallback / amount,
      estimatedTime: "5–30 min", fees: 0.4,
...
// ─────────────────────────────────────────────────────────────
// AGGREGATOR — parallel fetch, cache check, normalised output
// Returns:
//   best       — quote with highest amountOut
//   quotes     — all 3 normalised quotes sorted best→worst
//   minAmount  — real minimum with 10% buffer
...
async function fetchBestRate(from, to, amount) {
  ...
  const quotes = comparison
    .map((q) => ({
      ...
      estimatedTime: q.provider === "ChangeNOW" ? "2-5 min" : q.provider === "Exolix" ? "5-20 min" : "5-30 min",
      fees: 0.4,
      minAmount,
      quotaId: q.quotaId || "",
  ...
  const best = {
    ...
    estimatedTime: bestQuote.provider === "ChangeNOW" ? "2-5 min" : bestQuote.provider === "Exolix" ? "5-20 min" : "5-30 min",
    fees: 0.4,
    minAmount,
...
export default function AtlasSwapApp() {
  ...
  // ── Re-fetch prices instantly when fromCoin changes ──
  // This ensures the USD value shown under YOU SEND is always fresh
  useEffect(() => {
    fetchLivePrices();
